...
$cthreshold_value = 0;
if ($new_threshold != "") {
	$cthreshold_value = $new_threshold;
} else if (isset($_COOKIE['comment_threshold'])) {
	$cthreshold_value = $_COOKIE['comment_threshold'];
}

$pthreshold_value = 0;
if ($new_pthreshold != "") {
	$pthreshold_value = $new_pthreshold;
} else if (isset($_COOKIE['post_threshold'])) {
	$pthreshold_value = $_COOKIE['post_threshold'];
}
...
<input type="text" name="cthreshold" value="<?php echo $cthreshold_value; ?>"/>
</td></tr>
<tr><td>
<label class="block">Post Threshold</label><p>Any post with a score below this will be ignored.</p>
</td><td>
<input type="text" name="pthreshold" value="<?php echo $pthreshold_value; ?>"/>
</td></tr>
<tr><td>
